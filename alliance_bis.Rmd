...
Ce graphique semble suggérer que les scores de névrosisme seraient plus élevés lorsque les psychologues rapportent une alliance thérapeutique bonne ou très bonne alors que les score seraient inférieurs pour les alliances thérapeutiques inférieures. Pour vérifier cette impression, nous créons deux groupes : un premier groupe de détenus pour lesquelles l'alliance a été cotée 1 ou 2 (mauvaise alliance) et un second groupe pour lesquels l'alliance a été cotée 3 ou 4 (bonne alliance). Quels sont les scores de névrosisme pour ces deux groupes ?
...
Nous constatons toutefois que l'introduction de ces deux variables indépendants fragilise le modèle prédictif. Nous décidons dès lors de ne retenir que la variable la plus corrélée, c'est-à-dire N6.
...
corN1therapiep <- round(corN1therapie$p.value,2)
# N2
...
corN2therapiep <- round(corN2therapie$p.value,2)
# N3
...
corN3therapiep <- round(corN3therapie$p.value,2)
# N4
...
corN4therapiep <- round(corN4therapie$p.value,2)
# N5
...
corN5therapiep <- round(corN5therapie$p.value,2)
# N6
...
corN6therapiep <- round(corN6therapie$p.value,2)

tableNbiscor.df <- data.frame(Facette= c("N1","N2","N3","N4","N5","N6"), r = c(corN1therapier, corN2therapier, corN3therapier, corN4therapier,corN5therapier,corN6therapier), p = c(corN1therapiep, corN2therapiep, corN3therapiep, corN4therapiep, corN5therapiep, corN6therapiep))
```
...
Le tableau tableNbiscor indique quatre variables seraient significativement corrélées avec l'indication de prise en charge : N3 (Dépression), N4 (Timidité sociale), N6 (Vulnérabilité au stress) et N1 (Anxiété). Afin d'étudier ces liens de manière plus approfondie, nous construisons un modèle de régression logistique qui tente de prédire l'indication thérapeutique avec quatre variables indépendantes : N3, N4, N6 et N1.

```{r echo=FALSE}
# pris de https://perso.ens-lyon.fr/lise.vaudor/realiser-une-regression-logistique-avec-r/
N3N4N6N1therapiereg=glm(alliance.df$therapie~N3+N4+N6+N1, family=binomial(link=logit))
...
Selon Costa & MCCrae, p. 34 la variable N6 se définit ainsi :
...
Egalement selon Costa & MCCrae, p. 33 la variable N3 se définit ainsi :
